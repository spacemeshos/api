...
func RegisterPostServiceHandlerServer(ctx context.Context, mux *runtime.ServeMux, server PostServiceServer) error {
	mux.Handle(http.MethodPost, pattern_PostService_Register_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
...
// RegisterPostInfoServiceHandlerServer registers the http handlers for service PostInfoService to "mux".
// UnaryRPC     :call PostInfoServiceServer directly.
// StreamingRPC :currently unsupported pending https://github.com/grpc/grpc-go/issues/906.
...
func RegisterPostInfoServiceHandlerServer(ctx context.Context, mux *runtime.ServeMux, server PostInfoServiceServer) error {
	mux.Handle(http.MethodPost, pattern_PostInfoService_PostStates_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
...
// RegisterPostInfoServiceHandlerFromEndpoint is same as RegisterPostInfoServiceHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterPostInfoServiceHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterPostInfoServiceHandler(ctx, mux, conn)
}

// RegisterPostInfoServiceHandler registers the http handlers for service PostInfoService to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterPostInfoServiceHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterPostInfoServiceHandlerClient(ctx, mux, NewPostInfoServiceClient(conn))
}

// RegisterPostInfoServiceHandlerClient registers the http handlers for service PostInfoService
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "PostInfoServiceClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "PostInfoServiceClient"
// doesn't go through the normal gRPC flow (creating a gRPC client etc.) then it will be up to the passed in
// "PostInfoServiceClient" to call the correct interceptors. This client ignores the HTTP middlewares.
