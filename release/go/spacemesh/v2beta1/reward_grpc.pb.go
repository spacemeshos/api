...
func (c *rewardServiceClient) List(ctx context.Context, in *RewardRequest, opts ...grpc.CallOption) (*RewardList, error) {
	...
	out := new(RewardList)
	err := c.cc.Invoke(ctx, RewardService_List_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type RewardStreamServiceClient interface {
	Stream(ctx context.Context, in *RewardStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Reward], error)
}
...
func (c *rewardStreamServiceClient) Stream(ctx context.Context, in *RewardStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Reward], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &RewardStreamService_ServiceDesc.Streams[0], RewardStreamService_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[RewardStreamRequest, Reward]{ClientStream: stream}
...
type RewardStreamService_StreamClient = grpc.ServerStreamingClient[Reward]
...
type RewardStreamServiceServer interface {
	Stream(*RewardStreamRequest, grpc.ServerStreamingServer[Reward]) error
}

// UnimplementedRewardStreamServiceServer should be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
func (UnimplementedRewardStreamServiceServer) Stream(*RewardStreamRequest, grpc.ServerStreamingServer[Reward]) error {
	return status.Errorf(codes.Unimplemented, "method Stream not implemented")
}
func (UnimplementedRewardStreamServiceServer) testEmbeddedByValue() {}
...
func RegisterRewardStreamServiceServer(s grpc.ServiceRegistrar, srv RewardStreamServiceServer) {
	// If the following call pancis, it indicates UnimplementedRewardStreamServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
...
func _RewardStreamService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(RewardStreamServiceServer).Stream(m, &grpc.GenericServerStream[RewardStreamRequest, Reward]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type RewardStreamService_StreamServer = grpc.ServerStreamingServer[Reward]
